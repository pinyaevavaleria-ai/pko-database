...
function FinancialsSection({ company, details }: { company: RatingCompany; details: CompanyDetails }) {
  const fin2023 = details.financials.find(f => f.year === 2023);
  const fin2024 = details.financials.find(f => f.year === 2024);

  const rows: { label: string; value: number; prev: number | null; dot: string }[] = [
    { label: 'Выручка', value: details.revenue2024, prev: details.revenue2023, dot: '#0DF0E6' },
    { label: 'Прочие доходы', value: details.otherIncome2024, prev: details.otherIncome2023, dot: '#6366f1' },
    { label: 'Расходы', value: company.cost, prev: fin2023?.cost ?? null, dot: '#3b82f6' },
    { label: 'Чистая прибыль', value: company.profit, prev: fin2023?.profit ?? null, dot: '#22c55e' },
    { label: 'Фин. вложения', value: fin2024?.assets ?? 0, prev: fin2023?.assets ?? null, dot: '#f59e0b' },
    { label: 'Дебит. задолж.', value: company.receivable, prev: fin2023?.receivable ?? null, dot: '#f97316' },
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
        ...
        <span style={{ fontSize: '13px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: spaceGrotesk }}>
        ...
        return (
          <div key={label} style={{
            ...
            <div style={{ display: 'flex', gap: '40px', alignItems: 'center' }}>
              <span style={{ fontSize: '14px', fontWeight: 600, minWidth: '100px', textAlign: 'right' }}>
                {fmtMoney(value)} ₽
              </span>
              <span style={{
                fontSize: '13px', fontWeight: 600, minWidth: '60px', textAlign: 'right',
...
function DynamicsSection({ details }: { details: CompanyDetails }) {
  ...
  const fins = details.financials.filter(f => f.year >= 2020 && f.year <= 2024).sort((a, b) => a.year - b.year);
    ...
    <div style={{
      ...
      <div style={{ overflowX: 'auto', marginTop: '16px' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr>
              <th style={{ textAlign: 'left', padding: '8px 12px', borderBottom: `1px solid ${BORDER}`, color: MUTED, fontWeight: 500 }} />
              {fins.map(f => (
                <th key={f.year} style={{ textAlign: 'center', padding: '8px 12px', borderBottom: `1px solid ${BORDER}`, color: MUTED, fontWeight: 600 }}>
                  {f.year}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {CHART_METRICS.map(m => (
              <tr key={m.key}>
                <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BORDER}`, color: 'rgba(255,255,255,0.7)', whiteSpace: 'nowrap' }}>
                  {m.label}
                </td>
                {fins.map(f => (
                  <td key={f.year} style={{ textAlign: 'center', padding: '8px 12px', borderBottom: `1px solid ${BORDER}`, fontWeight: 500, fontFamily: spaceGrotesk }}>
...
function FundraisingSidebar({ details }: { details: CompanyDetails }) {
  ...
  return (
    <div style={{
      background: CARD_BG, borderRadius: '12px', border: `1px solid ${BORDER}`,
      padding: '20px', display: 'flex', flexDirection: 'column', alignSelf: 'start',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '13px' }}>
          {fr && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: MUTED }}>Тип</span>
                <span style={{ fontWeight: 500, textAlign: 'right', maxWidth: '180px' }}>{fr.type}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              {fr.founder && fr.founder !== '—' && (
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: MUTED }}>Учредитель</span>
                  <span style={{ fontWeight: 500, textAlign: 'right', maxWidth: '180px' }}>{fr.founder}</span>
                </div>
              )}
            </>
          )}
          ...
          {bonds && bonds.length > 0 && (
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: MUTED }}>Облигации</span>
                <span style={{ fontWeight: 600, fontFamily: spaceGrotesk }}>{bonds.length} вып.</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: MUTED }}>Общий объём</span>
                <span style={{ fontWeight: 600, fontFamily: spaceGrotesk }}>{totalVolume.toLocaleString('ru-RU')} млн ₽</span>
              </div>
              {bonds[0].coupon && (
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: MUTED }}>Купон</span>
                  <span style={{ fontWeight: 500 }}>{bonds[0].coupon}</span>
                </div>
              )}
              {firstRating && (
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: MUTED }}>Рейтинг</span>
                  <span style={{ fontWeight: 500, fontSize: '12px', textAlign: 'right', maxWidth: '180px' }}>{firstRating}</span>
                </div>
              )}
            </>
          )}
